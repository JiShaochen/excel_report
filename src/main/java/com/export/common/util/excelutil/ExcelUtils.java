...
public class ExcelUtils<E> {
    ...
    public static <T> void writeToFile(List<ManySheetDTO> data, ExcelDataFormatter edf, String filePath) throws Exception {
        // 创建并获取工作簿对象
        Workbook wb = getManySheetWorkBook(data, edf);
        // 写入到文件
        FileOutputStream out = new FileOutputStream(filePath);
        wb.write(out);
        out.close();
     ...
     * @throws Exception
     */
    public static <T> void writeToFile(ManySheetDTO<T> data, ExcelDataFormatter edf, String filePath) throws Exception {
        writeToFile(Arrays.asList(data), edf, filePath);
    }

    public static <T> Workbook getWorkBook(ManySheetDTO<T> data, ExcelDataFormatter edf, String filePath) throws Exception {
        return getManySheetWorkBook(Arrays.asList(data), edf);
    }

    public static <T> Workbook getWorkBook(List<ManySheetDTO> data, ExcelDataFormatter edf, String filePath) throws Exception {
        return getManySheetWorkBook(data, edf);
    }

    private static <T> Workbook getManySheetWorkBook(List<ManySheetDTO> manySheetDTOS, ExcelDataFormatter edf) throws Exception  {
        // 创建工作簿
        Workbook wb = new SXSSFWorkbook();
        for (ManySheetDTO item : manySheetDTOS) {
            List<T> list = item.getData();
